...
export default function PurchaseOrderDetailCopy() {
  ...
  const handleOnBlur = (index) => {
    ...
    if(currentRow.quantity !== null && currentRow.unit_price !== null && currentRow.discount !== null){
      ...
      const updatedRows = rows.map((row, i) => i === index ? { ...row, amount: calculatedAmount } : row);
      setRows(updatedRows);
  ...
  const handleDelete = (index) => {
    ...
    const handleSaveAll = (po_id) => {
      axiosClient.post('/posaveAll/'+po_id, rows)
            .then(response => {
        ...
        <div className="flex justify-between items-center pb-2" style={{ borderBottom: '1px solid grey' }}>
          ...
          <div className="pt-8">
              <div className="flex w-full flex-wrap md:flex-nowrap p-2" >
                  ...
                  <Button className="bg-green-300" onClick={post} hidden={!disabledView || poData.status !== 'Draft' && poData.status !== 'Returned'}>
                    Post
                  </Button>
              </div>
                        ...
                        <ModalContent>
                          {(onClose) => (
                            <>
                              ...
                              <ModalBody>
                                <TableSelect columns={columnsCompanies} apiname={'allcompanies'} handleAction={handleVendor}>
                                </TableSelect>
                              </ModalBody>
                            </>
                          )}
                        ...
                        <ModalContent>
                          {(onClose) => (
                            <>
                              ...
                              <ModalBody>
                                <TableSelect columns={columnsCompanies} apiname={'allcompanies'} handleAction={handleShipTo}>
                                </TableSelect>
                              </ModalBody>
                            </>
                          )}
                                  ...
                                  <ModalContent>
                                    {(onClose) => (
                                      <>
                                        ...
                                        <ModalBody>
                                          <TableSelect columns={columnsItems} apiname={'ppbDetailSelect'} handleAction={(data) => handleItems(data)}>
                                          </TableSelect>
                                        </ModalBody>
                                      </>
                                    )}
